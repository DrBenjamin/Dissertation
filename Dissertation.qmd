...
![@fig-move-ai: Move AI advertising accurate markerless motion capturing on their website.](literature/Media/Move%20AI.png){#fig-move-ai fig-align="right"}
...
![@fig-klein-vogelbach-posture: 3 types of posture types defined by Klein-Vogelbach, the underlying functional movement concept was developed between 1955-1975.](literature/Media/3_posture_types.png){#fig-klein-vogelbach-posture}
...
: @tab-software-framework-service-stack: Software, Framework and Service Stack used for the development, training, deployment, and automation of the system.
...
:   @lst-environment-manifests: Environment manifest files used for local development and containerization.{#lst-environment-manifests}
...
: @lst-conda-env-setup: Reproducible Conda environment setup for local execution.{#lst-conda-env-setup}
...
Streamlit is widely used in the data science sphere, to build interactive data apps. The same Interface was used to test the build classification models, as you can see in @fig-streamlit-app.

![@fig-streamlit-app.: Streamlit Web App with classification of postures using the build quantized model from Training 4 (3-class) @fig-training-3class-4.](docs/Streamlit_WebApp.png){#fig-streamlit-app}

The app can digest images from the Browser as User upload and from the self-hosted MinIO server. As later described, a n8n automation was processing images automatically from a MinIO bucket (and saved it back to a second bucket), the App uses the same API to classify or annotate images (when a pose model is chosen). This lead to a more efficient development, as only on source of truth for the image detection/processing code was needed.
...
: @lst-research-client-example: Example usage of the custom TFLite image classifier script for testing a trained model on an example image.{#lst-research-client-example}

As a counterpart to the Streamlit Web App there is also a command line tool for testing (also utilizing the API). Example:

``` bash
# Example usage of the human posture analysis script in image mode, calling the MediaPipe API for inference and saving the output with pose overlay.
...
: @lst-human-posture-analysis-example: Example usage of the human posture analysis script in image mode, calling the MediaPipe API for inference and saving the output with pose overlay.{#lst-human-posture-analysis-example}
...
:   @lst-dockerfiles: Dockerfiles for containerization of the training environment.{#lst-dockerfiles}
...
: @lst-docker-compose: Development environment with Docker Compose, building the necessary images and running the defined services (e.g., MediaPipe API, Streamlit Web App).{#lst-docker-compose}
...
The @fig-CNN-workflow illustration shows the whole implementation setup and dependencies.

![@fig-CNN-workflow: Implementation map illustrates the pipelines, automation and containerization.](docs/CNN%20Workflow.drawio.png){#fig-CNN-workflow}
...
: @tab-training-2class-1: Confusion matrix for 2-class training 1 showing class imbalance and preference for `non-normal posture`.{#tab-training-2class-1}
...
: @tab-training-row-2class-1: Row-normalized confusion matrix for 2-class training 1 showing class imbalance and preference for `non-normal posture`.{#tab-training-row-2class-1}
...
: @tab-report-training-2class-1: Classification results for 2-class training 1 showing an accuracy of 60%, but on a highly imbalanced class distribution.{#tab-report-training-2class-1}

![@fig-training-2class-1: Confusion matrix showing low amount of test images and preference for category \`non-normal posture\`.](literature/Results/Training_2-class_1.png){#fig-training-2class-1}
...
: @tab-training-2class-2: Confusion matrix for 2-class training 2 showing class imbalance and preference for `normal posture`.{#tab-training-2class-2}
...
: @tab-training-row-2class-2: Row-normalized confusion matrix for 2-class training 2 showing class imbalance and preference for `normal posture`.{#tab-training-row-2class-2}
...
: @tab-report-training-2class-2: Classification results for 2-class training 2 showing an accuracy of 53.16%.{#tab-report-training-2class-2}

![@fig-training-2class-2: Confusion matrix showing \`normal posture\` class preference.](literature/Results/Training_2-class_2.png){#fig-training-2class-2}
...
: @tab-training-2class-3: Confusion matrix for 2-class training 3 showing class imbalance and preference for `normal posture`.{#tab-training-2class-3}
...
: @tab-training-row-2class-3: Row-normalized confusion matrix for 2-class training 3 showing class imbalance and preference for `normal posture`.{#tab-training-row-2class-3}
...
: @tab-report-training-2class-3: Classification results for 2-class training 3 showing an accuracy of 55.85%.{#tab-report-training-2class-3}

![@fig-training-2class-3: Confusion matrix again showing \`normal posture\` class preference.](literature/Results/Training_2-class_3.png){#fig-training-2class-3}
...
: @tab-training-3class-1: Confusion matrix for 3-class training 1 showing slight over-prediction of `slumped` class.{#tab-training-3class-1}
...
: @tab-training-row-3class-1: Row-normalized confusion matrix for 3-class training 1 showing slight over-prediction of `slumped` class.{#tab-training-row-3class-1}
...
: @tab-report-training-3class-1: Classification results for 3-class training 1 showing an accuracy of 30.65% and a balanced accuracy of 31.11%.{#tab-report-training-3class-1}

![@fig-training-3class-1: Confusion matrix showing under-representation of \`tensed posture\` class.](literature/Results/Training_3-class_1.png){#fig-training-3class-1}
...
: @tab-training-3class-2: Confusion matrix for 3-class training 2 showing under-representation of `normal posture` class and a preference for `slumped posture`.{#tab-training-3class-2}
...
: @tab-training-row-3class-2: Row-normalized confusion matrix for 3-class training 2 showing under-representation of `normal posture` class and a preference for `slumped posture`.{#tab-training-row-3class-2}
...
: @tab-report-training-3class-2: Classification results for 3-class training 2 showing an accuracy of 32.16% and a balanced accuracy of 32.63%.{#tab-report-training-3class-2}

![@fig-training-3class-2: Confusion matrix showing under-representation of \`normal posture\` class and a preference for \`slumped posture\`.](literature/Results/Training_3-class_2.png){#fig-training-3class-2}
...
: @tab-training-3class-3: Confusion matrix for 3-class training 3 showing preference for `slumped posture` class.{#tab-training-3class-3}
...
: @tab-training-row-3class-3: Row-normalized confusion matrix for 3-class training 3 showing preference for `slumped posture` class.{#tab-training-row-3class-3}
...
: @tab-report-training-3class-3: Classification results for 3-class training 3 showing an accuracy of 39.20% and a balanced accuracy of 39.79%.{#tab-report-training-3class-3}

![@fig-training-3class-3: Confusion matrix showing preference of \`slumped posture\` class.](literature/Results/Training_3-class_3.png){#fig-training-3class-3}
...
: @tab-training-3class-4: Confusion matrix for 3-class training 4 showing preference for `slumped posture` class.{#tab-training-3class-4}
...
: @tab-training-row-3class-4: Row-normalized confusion matrix for 3-class training 4 showing preference for `slumped posture` class.{#tab-training-row-3class-4}
...
: @tab-report-training-3class-4: Classification results for 3-class training 4 showing an accuracy of 41.21% and a balanced accuracy of 41.67%.{#tab-report-training-3class-4}

![@fig-training-3class-4: Confusion matrix showing again preference of \`slumped posture\` class.](literature/Results/Training_3-class_4.png){#fig-training-3class-4}

### Error analysis

Across all experiments a consistent pattern of misclassification is apparent. The models show relatively high recall for the `slumped posture` class but lower precision, indicating a tendency to predict `slumped` for a wide range of inputs exemplified visible in @fig-training-3class-3. `Normal posture` is frequently misclassified (low recall in several runs), and `tensed posture` predictions vary by architecture: deeper Lite variants tended to improve overall discrimination.
...
@fig-overlay-mediapipe: Google MediaPipe Pose overlay to show angles of head/neck and torso.

![Overlay annotation by Google MediaPipe Pose.](literature/Media/Overlay_annotation.jpg){#fig-overlay-mediapipe}
