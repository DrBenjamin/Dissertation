...
The Streamlit Web App can utilize two different kind of CNN models. The first supported type of models are the existing MediaPipe Pose landmarker models to create images with 33 3-dimensional pose mapping as an overlay on the images (as shown in @fig-pose_landmarks_index) . These come in three different bundle variants (Lite, Full, Heavy), which differ in model complexity, computational requirements, and performance (accuracy). As inference is not performed directly in the browser on the user computer or mobile device, this web application can be shipped to any device, even older devices without high computational resources.
...
**Legend**:** The red dots show the 33 3-dimensional pose mapping (numbered 0-32) of the Google MediaPipe Pose landmarker model, while the black lines indicate the extremities.
...
**Legend:** This image shows a correct annotation of an image taken from the `normal posture` class.
...
**Legend:** This image shows an incorrect annotation of an image of the `slumped posture` class, where the pelvis and shoulder angles are misclassified.
